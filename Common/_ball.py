...
def random_rot(dim, dtype):
    ...
    H = np.eye(dim, dtype=dtype)
    D = np.ones((dim,), dtype=dtype)
    for n in range(1, dim):
        x = np.random.standard_normal(size=(dim - n + 1,)).astype(dtype)
        D[n - 1] = np.sign(x[0])
        x[0] -= D[n - 1] * np.sqrt((x * x).sum())
        # Householder transformation
        Hx = np.eye(dim - n + 1, dtype=dtype) - 2.0 * np.outer(x, x) / (x * x).sum()
        mat = np.eye(dim, dtype=dtype)
        mat[n - 1 :, n - 1 :] = Hx
        H = np.dot(H, mat)
